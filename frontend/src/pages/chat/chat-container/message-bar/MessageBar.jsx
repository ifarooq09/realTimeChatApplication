...
const MessageBar = () => {
  ...
  const handleAttachementChange = async (event) => {
    try {
      ...
      if (file) {
        ...
        setIsUploading(true)
        const response = await apiService.post(UPLOAD_FILE_ROUTE, formData, {
  ...
  return (
    <div className="h-[10vh] bg-[#1c1d25] flex justify-center items-center px-8 mg-6 gap-6">
      ...
      <button
        className="bg-yellow-500 rounded-md flex items-center justify-center p-5 focus:border-none hover:bg-yellow-400 focus:bg-yellow-400 focus:outline-none focus:text-white duration-300 transition-all"
